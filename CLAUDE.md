...
Untuk Tab 5 (AI Advisor) — jalankan tambahan di **host** (bukan Docker):
```bash
python3 scripts/hermes_advisor_bridge.py --port 8090 &
```
Tanpa ini, Tab 5 tetap tampil (data teknikal dari ClickHouse), cuma narasi AI-nya absen.
...
- **AI Advisor — backend (Docker) tidak bisa langsung panggil Hermes (host)**: `bro_analysis` cuma ada di PATH HOST (`~/.local/bin`), backend jalan di CONTAINER — filesystem beda, subprocess langsung/sys.path-import ala contoh lama di `docs/hermes-integration.md` TIDAK JALAN tanpa volume-mount yang rapuh. Solusi: `scripts/hermes_advisor_bridge.py` — HTTP kecil (stdlib only) yang jalan di HOST, backend panggil via `http://host.docker.internal:8090` (`extra_hosts: host-gateway` sudah di-set di `docker-compose.yml` service backend, WAJIB di Linux krn `host.docker.internal` tidak otomatis resolve). Pola ini identik dgn `LLM_BASE_URL` (Tab 3) — HTTP, bukan proses lokal.
